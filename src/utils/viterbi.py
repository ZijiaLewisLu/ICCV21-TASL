...
class Viterbi(object):
    ...
    def __init__(self, grammar: Grammar, length_model: PoissonModel, prior_model: Prior, 
            frame_sampling = 1, max_hypotheses = np.inf):
    ...
    def decode(self, log_frame_probs): 
        ...
        # subtract log_prior and max
        log_frame_probs = log_frame_probs - self.prior_model.log_prior()
        ...
        self.final_hyp, self.all_hyp = final_hyp, hyps
    ...
    def decode_frame(self, t, old_hyp, frame_scores):
        ...
        for key, hyp in old_hyp.items():
            ...
            for new_label in self.grammar.possible_successors(context):
                ...
                score = hyp.score + self.frame_score(frame_scores, t, label) + self.grammar.score(context, new_label)
                score += self.length_model.score(length, label)
    ...
    def finalize_decoding(self, old_hyp):
        ...
        for key, hyp in old_hyp.items():
            context, label, length = key[0:-2], key[-2], key[-1]
            context = context + (label,)
            score = hyp.score + self.grammar.score(context, self.grammar.end_symbol())
            score += self.length_model.score(length, label) 
            if score >= final_hyp.score:
                final_hyp.score, final_hyp.traceback = score, hyp.traceback
                self.final_trans = context
        # return final hypothesis
        return final_hyp

    def traceback(self, hyp: HypDict.Hypothesis, n_frames):
        ...
        while traceback is not None:
            ...
            labels.extend([traceback.label] * self.frame_sampling)
            if traceback.boundary and traceback.predecessor is not None:
        ...
        labels.extend( [labels[-1]] * (n_frames - len(labels)) ) # append labels for missing frames
    ...
    def init_incremental_forward(self):
        hyps = self.HypDict()
        key = -1
        # hyps.update(key, torch.Tensor([0.]).cuda(), None, recons_error=torch.Tensor([0.]).cuda())
