...
class PathGrammar(Grammar):
    ...
    def __init__(self, transcript_file_or_list, label2index_map):
        self.num_classes = len(label2index_map)
        if isinstance(transcript_file_or_list, str):
            transcripts = self._read_transcripts(transcript_file_or_list, label2index_map)
        else:
            transcripts = transcript_file_or_list
